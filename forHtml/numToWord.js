...
function htmlRender(){
    let elements = document.getElementsByClassName('num-to-word');
    for(let counter = 0; counter < elements.length; counter++){
        let numberToArray = elements[counter].innerHTML.toString().split('').reverse();
        let result = render(numberToArray);
        elements[counter].innerHTML = result;
    }
